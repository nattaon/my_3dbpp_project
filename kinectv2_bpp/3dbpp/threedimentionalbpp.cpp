#include "threedimentionalbpp.h"

ThreeDimentionalBPP::ThreeDimentionalBPP()
{
...
void ThreeDimentionalBPP::error(char *str, ...)
{
  va_list args;
...
void ThreeDimentionalBPP::timer(double *time)
{
...
/* test for time limit */
void ThreeDimentionalBPP::check_timelimit(long max)
{
  double t;
  if (max == 0) return;
  timer(&t);
...
/* test for node limit */
void ThreeDimentionalBPP::check_nodelimit(long nodes, long max)
{
  if (max == 0) return;
  if (nodes >= max) {
...
void ThreeDimentionalBPP::check_iterlimit(long iterations, long max)
{
  if (max == 0) return;
  if (iterations >= max) {
...
int ThreeDimentionalBPP::dcomp(box *a, box *b)
{ int r; r = b->d - a->d; if (r != 0) return r; else return b->no - a->no; }
int ThreeDimentionalBPP::hcomp(box *a, box *b)
{ int r; r = b->h - a->h; if (r != 0) return r; else return b->no - a->no; }
int ThreeDimentionalBPP::vcomp(box *a, box *b) /* volume decr. */
{ int r; r = b->vol-a->vol; if (r != 0) return r; else return b->no - a->no; }
int ThreeDimentionalBPP::xcomp(heurpair *a, heurpair *b) /* depth decr. */
{ int r; r = b->d - a->d; if (r != 0) return r; else return b->lno - a->lno; }
int ThreeDimentionalBPP::lcomp(heurpair *a, heurpair *b) /* layer number decr. */
...
void *ThreeDimentionalBPP::palloc(long sz, long no)
{
  long size;
  void *p;
...
void ThreeDimentionalBPP::pfree(void *p)
{
...
void ThreeDimentionalBPP::checksol(allinfo *a, box *f, box *l)
{
  box *i, *j, *m;
...
void ThreeDimentionalBPP::savesol(allinfo *a, box *f, box *l, ntype z)
{
  box *i, *k, *m;
...
void ThreeDimentionalBPP::isortincr(int *f, int *l)
{
  register int mi;
  register int *i, *j, *m;
  register int d;
...
void ThreeDimentionalBPP::isortdecr(int *f, int *l)
{
  register int mi;
  register int *i, *j, *m;
  register int d;
...
void ThreeDimentionalBPP::psortdecr(point *f, point *l)
{
  register point mi;
...
int ThreeDimentionalBPP::bound_zero(allinfo *a, box *f, box *l)
{
  box *i, *m;
  stype vsum, lb;
...
void ThreeDimentionalBPP::rotate_solution(allinfo *a, box *f, box *l)
{
  register box *i, *m;
  register itype w, x;
...
void ThreeDimentionalBPP::rotate_problem(allinfo *a, box *f, box *l)
{
  register box *i, *m;
  register itype w, x;
...
void ThreeDimentionalBPP::choose_boxes(allinfo *a, box *f, box *l, int W2, int D2,
          box *fbox , box **lbox)
{
  box *i, *k, *m;
...
void ThreeDimentionalBPP::find_plist(box *fbox, box *lbox, itype M, int dim, int *pl)
{
  register box *i, *m;
  register int *k, *j, *l;
...
int ThreeDimentionalBPP::bound_one_x(allinfo *a, box *f, box *l)
{
  register box *i, *m;
...
int ThreeDimentionalBPP::bound_one(allinfo *a, box *f, box *l)
{
  int i, lb, lbx;
...
int ThreeDimentionalBPP::bound_two_x(allinfo *a, box *f, box *l)
{
  register box *i, *m;
...
int ThreeDimentionalBPP::bound_two(allinfo *a, box *f, box *l)
{
  int i, lb, lbx;
...
void ThreeDimentionalBPP::onelayer(allinfo *a, box *f, box *m, box *l, int d)
{
  int s, t;
...
box *ThreeDimentionalBPP::countarea(box *f, box *l, stype barea)
{
  box *i;
  stype area, d;
...
box *ThreeDimentionalBPP::remboxes(box *f, box *l, itype *depth)
{
  box *i, *j;
  itype d;
...
void ThreeDimentionalBPP::assignboxes(heurpair *t, heurpair *u, ntype maxbin, box *f, box *l)
{
  box *i, *m;
  heurpair *h;
  itype b, z;
...
void ThreeDimentionalBPP::onedim_binpack(heurpair *i, heurpair *f, heurpair *l,
...
void ThreeDimentionalBPP::dfirst_heuristic(allinfo *a)
{
  box *j, *f, *l, *m;
...
void ThreeDimentionalBPP::dfirst3_heuristic(allinfo *a)
{
  int i;
  double t1, t2;
...
void ThreeDimentionalBPP::modifyandpush(int i, int j, int rel, boolean dom)
{
  dompos->i = i;
  dompos->j = j;
  dompos->domain = dom;
...
void ThreeDimentionalBPP::popdomains(domainpair *pos)
{
  for (; dompos != pos; ) {
    dompos--;
    if (dompos->domain) {
...
boolean ThreeDimentionalBPP::findcoordinates(allinfo *a, int n, box *f)
{
  register box *g, *h;
  register int sum;
  int i, j, k, W, H, D;
...
void ThreeDimentionalBPP::checkdomain(allinfo *a, int i, int j, int n, box *f, int value)
{
...
boolean ThreeDimentionalBPP::reducedomain(allinfo *a, int n, box *f)
{
...
void ThreeDimentionalBPP::recpack(allinfo *a, int i, int j, int n, box *f, int rel)
{
  int i1, j1;
  domainpair *pos;
  boolean feas;
...
boolean ThreeDimentionalBPP::general_pack(allinfo *a, box *f, box *l)
{
...
boolean ThreeDimentionalBPP::onebin_general(allinfo *a, box *f, box *l, boolean fast)
{
  boolean solution;
  double t1, t2;
...
void ThreeDimentionalBPP::envelope(point *f, point *l, point *s1, point **sm,
...
void ThreeDimentionalBPP::checkdom(point *s1, point *sl, point *sm)
{
...
point *ThreeDimentionalBPP::removedom(point *s1, point *sl)
{
...
void ThreeDimentionalBPP::initboxes(box *f, box *l, point *fc, point **lc,
...
stype ThreeDimentionalBPP::findplaces(allinfo *a, box *f, box *l,
...
void ThreeDimentionalBPP::branch(allinfo *a, box *f, box *l, int miss, stype fill)
{
  register box *i;
  register point *s;
  int d;
...
boolean ThreeDimentionalBPP::onebin_robot(allinfo *a, box *f, box *l, boolean fast)
{
  box *i, *j, *m;
  stype vol;
  double t1, t2;
...
void ThreeDimentionalBPP::mcut_heuristic(allinfo *a)
{
  box *f, *l, *i, *j, *m;
  int b, n;
...
void ThreeDimentionalBPP::mcut3_heuristic(allinfo *a)
{
  int i;
  double t1, t2;
...
boolean ThreeDimentionalBPP::fits2(box *i, box *j, itype W, itype H, itype D)
{
...
boolean ThreeDimentionalBPP::fits2p(box *i, box *j, itype W, itype H, itype D)
{
...
boolean ThreeDimentionalBPP::fits3(box *i, box *j, box *k, itype W, itype H, itype D, int packtype)
{
  box *t;
  itype w, h, d, r;
...
boolean ThreeDimentionalBPP::fitsm(allinfo *a, box *t, box *k, boolean fast)
{
  boolean fits;
  ntype lb;
...
boolean ThreeDimentionalBPP::onebin_decision(allinfo *a, box *j, int bno)
{
  register box *i, *k, *m;
  box t[MAXBOXES];
  boolean fits;
...
boolean ThreeDimentionalBPP::onebin_heuristic(allinfo *a, box *f, box *l)
{
  box *i, *m;
  boolean fits;
...
boolean ThreeDimentionalBPP::try_close(allinfo *a, box **curr, ntype bno,
...
void ThreeDimentionalBPP::free_close(allinfo *a, ntype bno,
                box *oldf, box *oldl, box *oldlc, ntype oldnoc,
                boolean *oldclosed)
{
...
void ThreeDimentionalBPP::rec_binpack(allinfo *a, box *i, int bno, ntype lb, int level)
{
...
void ThreeDimentionalBPP::clearboxes(allinfo *a)
{
  box *i, *m;
...
void ThreeDimentionalBPP::copyboxes(allinfo *a, int *w, int *h, int *d, int W, int H, int D)
{
  box *i, *m;
  int k;
...
void ThreeDimentionalBPP::returnboxes(allinfo *a, int *x, int *y, int *z, int *bno)
{
  box *i, *m;
  int k;
...
void ThreeDimentionalBPP::binpack3d(int n, int W, int H, int D,
               int *w, int *h, int *d,
...
void ThreeDimentionalBPP::printboxes_array(int size, int* w, int* h, int* d, int* bno, int* x, int* y, int* z)
{
  printf("\nprint box array\n");
